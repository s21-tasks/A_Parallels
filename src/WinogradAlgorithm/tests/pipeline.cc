...
void Test(unsigned int lines, unsigned int matrix_size, unsigned int repeat) {
    ...
    unsigned int k1 [[maybe_unused]] = 0, k2 [[maybe_unused]] = 0, k3 [[maybe_unused]] = 0;
    auto result = Time::Compare<Unit>(repeat, [&] {
        pipe.Execute(tasks[k1++]);
    }, [&] {
        for (auto &w : workers) {
    ...
    std::cout << "\033[1;32m";
    std::cout << "\nTest " << ++counter << " {Matrix size = " << matrix_size << ". Lines = " << lines << "}:\n";
    std::cout << "\033[1;37m";
    std::cout << '\t' << result[0] << ' ' << Time::Prefix<Unit>::val << "\t - Pipeline Winograd\n";
    std::cout << '\t' << result[1] << ' ' << Time::Prefix<Unit>::val << "\t - Basic Winograd\n";
    std::cout << '\t' << result[2] << ' ' << Time::Prefix<Unit>::val << "\t - Parallel\n";
}
