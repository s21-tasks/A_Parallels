...
template <class T>
void Winograd<T>::Mul(const M &A, const M &B, M &C, i_type odd_cap,
                      i_type winograd_cap) {
  i_type n = A.GetCols();
  if (n <= 1 || B.GetCols() != n || C.GetCols() != n || A.GetRows() != n ||
      B.GetRows() != n || C.GetRows() != n) {
...
struct Winograd<T>::LevelEven : public Level, public PointerBase {
  ...
  using PB::A11, PB::A12, PB::A22, PB::B11, PB::B21, PB::B22, PB::S1, PB::S2,
      PB::S3, PB::S4, PB::T1, PB::T2, PB::T3, PB::T4, PB::R1, PB::R2, PB::R3,
      PB::R4, PB::R5, PB::R6, PB::R7;
  using PB::n;
...
void Winograd<T>::Execute(const M &A, const M &B, M &C) {
  if (A.GetCols() != n_ || B.GetCols() != n_ || C.GetCols() != n_ ||
    ...
    throw std::invalid_argument(
        "Matrix size not match " + std::to_string(n_) +
        " != " + std::to_string(A.GetCols()) + " || " +
        std::to_string(B.GetCols()) + " || " + std::to_string(C.GetCols()) +
        " || " + std::to_string(A.GetRows()) + " || " +
...
void Winograd<T>::LevelOdd::SW(const T *A, const T *B, T *C) {
  ...
  for (i_type i = 0, in = n; i < n; ++i, ++in) {
    for (i_type j = 0, jn = n; j < n; ++j, ++jn) {
      ...
      if (!(jn == n * 2 - 1)) C[i * cn + jn] = r165 + R3[i * n + j];
      if (!(in == n * 2 - 1)) C[in * cn + j] = r16 - R4[i * n + j] + r7;
      if (!(in == n * 2 - 1) && !(jn == n * 2 - 1)) C[in * cn + jn] = r165 + r7;
    }
  }
#ifdef LEVEL_LOAD_TEST__P
  level_load[n] += Time::Duration<Time::ns>(time_point);
