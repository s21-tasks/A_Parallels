...
int main() {
    ...
    BLAS::Matrix<float> M1D(100, 500, [&] { return s21::Random::Normal<float>(0.0, 15.0); });
    BLAS::Matrix<float> M1C(M1D);
    BLAS::Matrix<float> M1(M1D);
    BLAS::Matrix<float> M2(1, 500, [&] { return s21::Random::Normal<float>(0.0, 5.0); });

    ThreadManager TM(thread_count);

    auto cr = s21::Time::Compare<s21::Time::mcs>(10000, [&] {
        ...
        ThreadManager::DispThreads(thread_count, ThreadManager::LoopThreads(thread_count, M1D.GetRows(), [&] (int k) {
