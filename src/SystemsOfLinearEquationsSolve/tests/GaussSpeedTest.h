  ...
  public:
    GaussSpeedTest() {
      ...
      auto home_menu = AddMenu(
              {"loading the blas from console",
               "set random plas",
               "print blas",
               "solve blas using usual method",
               "solve  blas using parallel method",
    ...
    void RandomFill(unsigned capacity) {
      Matrix<double> matr(capacity, capacity, [&] { return Random::Normal<double>(-100, 100); });
      std::vector<double> consts(capacity);
      std::generate(consts.begin(), consts.end(), [] {return Random::Uniform(-100,100);});
