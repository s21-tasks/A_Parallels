...
SLAEGauss::SLAEGauss(const Matrix<double>& m,
                     const std::vector<double>& consts) {
  set_equations(m, consts);
}

void SLAEGauss::set_equations(const Matrix<double>& m,
                              const std::vector<double>& consts) {
...
std::vector<double> SLAEGauss::UsualExecute() {
  ...
  for (int i = n - 1; i >= 0; --i) {
    double sum = 0;
    for (int j = i + 1; j < n; ++j) sum += matrix[i][j] * solution[j];
  ...
  for (int k = 0; k < n - 1; ++k) {
    ...
    for (int tr = 0; tr < threads; ++tr) {
      t.emplace_back([k, this, iters, tr] {
        for (int i = (k + 1) + (iters * tr); i < (k + 1) + (iters * (tr + 1));
             ++i) {
    ...
    for (int ii = 0; ii < t.size(); ++ii) t[ii].join();
    t.clear();
  }
...
void SLAEGauss::Print() {
  for (int i = 0; i < matrix.GetRows(); ++i) {
    for (int j = 0; j < matrix.GetCols(); ++j) {
      ...
      std::cout << std::fabs(matrix[i][j]) << " * "
                << static_cast<char>(97 + i);
    }
